...
| `order.order.not_pending` | 422 | El pedido ya está `Approved` o `Cancelled`: no admite comprobantes (`/order/proofs`), productos (`/order/items`), edición (`PUT /orders/{orderId}` y su `/preview`) ni otra aprobación |
