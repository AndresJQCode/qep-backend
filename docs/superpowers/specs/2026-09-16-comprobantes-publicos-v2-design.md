...
7. **Revisión final de la rama (D9, D16, D19).**
   - **Movimiento.** Un pedido puede quedar guardado con su evento aunque el request falle esperando el COMMIT (conexión cortada, pestaña cerrada), y el rollback del handler ya borró la copia pública. Por eso `PaymentProofMoveProcessor` verifica con `IPublicObjectStorage.ExistsAsync` (en R2, un HEAD; sólo el 404 es «no existe») que la copia exista antes de borrar el temporal. Si no existe, salta la entrada con un `Warning`: el temporal y el recurso quedan sin mover, las demás entradas siguen y el mensaje se marca. Un error al verificar es transitorio y el mensaje vuelve. Los handlers siguen guardando con el token del request.
   - **D16 por referencia.** `OrderPaymentProofResolver` rechaza con `order.payment_proof.file_not_available` un `PaymentProof` que ya usa otro comprobante de cualquier pedido (`IOrderRepository.IsPaymentProofFileInUseAsync`, sobre `IX_order_payment_proofs_file`), aunque todavía no se haya movido. La excepción es el comprobante que se reemplaza con su propio archivo. `QuotationFileRef.IsPaymentProof` lo informa, y los archivos `User` (D13) no tienen la regla.
   - **Duplicados en un request.** Los validadores de convertir y de sumar comprobantes rechazan con `validation.failed` el mismo `FileId` repetido (entre los nuevos y los `NewFileId` de reemplazo) y el mismo `ProofId` repetido.
   - **Retiro.** Para un `PaymentProof`, si la clave que trae el evento no está vacía y es distinta de la `PublicStorageKey` del archivo, `PaymentProofDetachProcessor` borra esa copia y audita `storage.public_object.purged` / `payment_proof_detached` cuando ninguna `IPublicObjectReferenceProbe` la referencia. Aplica aunque el archivo quede retenido, ya esté movido, purgado o en otro estado. Sin sondas de claves públicas, la copia se conserva y queda un `Warning`.
