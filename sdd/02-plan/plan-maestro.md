...
**No hay slice activo en este repositorio, y tampoco hay candidato en cola.** `CAT-09` cerró el
2026-08-18 y con él se agotó la lista: `catalog` quedó **simétrico** —`Product` y `TaxRate` tienen
los dos su `activate`— y la galería, que era el último pendiente heredado, está construida.

**Queda una decisión abierta que sí es trabajo: `SDD-OD-23`**, el filtro `status` del listado de
archivos que ante un valor inválido devuelve la lista **sin filtrar**. Es de la capability
`Storage`, y por `AGENTS.md` §4a se corrige dentro del slice que lo necesite o en un slice hermano
—nunca en silencio—. Hoy no bloquea a nadie.

> **Corrección del 2026-08-18: acá decía que había que abrir `storage` como módulo. Es falso.**
>
> Esta sección afirmaba que `Storage` «va por su segunda modificación desde afuera y sigue sin
> ficha, sin gate y sin prefijo», y planteaba como próxima decisión «¿se abre `storage` como
> módulo?». **El método dice lo contrario, y por escrito:**
>
> - `01-contexto/registro-de-modulos.md` § «Capabilities de plataforma» nombra a **`Storage`** en
>   la lista: «No son módulos de este proyecto. Se **consumen**, no se reescriben».
> - `00-metodo/apertura-de-modulo.md` § «Cuándo NO abrir un módulo» lo pone como la **primera** de
>   sus cuatro señales: «infraestructura que varios módulos usan igual → capability de plataforma;
>   se consume, no se crea». Y aclara que las cuatro «salieron de casos reales de este proyecto».
>
> **`CAT-05` y `CAT-09` no tomaron ningún atajo:** hicieron lo que el método define para una
> capability. La deuda que este ledger declaró no existía.
>
> **Lo que sí faltaba, y era real:** `AGENTS.md` §4a exige que el defecto de una capability se
> registre como `SDD-OD-*` con evidencia de código. `CAT-09` cumplió las otras tres condiciones
> —corrección acotada dentro del slice, y RED en el proyecto de pruebas de la capability— pero
> **nunca registró el `SDD-OD-*`**. Se corrige con `SDD-OD-22` y `SDD-OD-23`, abajo.
>
> Verificado contra `apertura-de-modulo.md`, `registro-de-modulos.md` y `AGENTS.md` §4a el
> 2026-08-18. `SDD-ADR-01`: gana el método escrito.

**Y lo que le toca al otro repo, que ya no depende de acá:** el gate declara **dos** operaciones
que el frontend no consume —`activate` de `products` y de `tax-rates`—. Sin ellas la UI no tiene
cómo deshacer una desactivación, y en el caso de las tasas tampoco cómo rescatar una que quedó
inactiva y en uso. El backend ya las expone; el botón es fila del ledger de `qep-frontend`.

**La obligación de método que `CAT-07` dejó y `CAT-08` cumplió:** revisar **antes** de publicar.
`CAT-07` publicó primero y su gate `pre-push` respondió `allow` con «the publication range is
empty» — no gobernó nada. `CAT-08` validó antes, con `base_relationship_valid: true`. Se mantiene.
...
Las de este repositorio. Formato y numeración en
`qep-frontend/sdd/00-metodo/convenciones-de-id.md`: **`SDD-OD-*` es una serie global entre los dos
repos**, así que un ID nuevo se toma verificando el máximo en **todas las ramas de los dos**, no
sólo en `develop`. El 2026-08-18 el máximo ocupado era `SDD-OD-21`, en una rama de `qep-frontend`
que todavía no está mergeada.

`AGENTS.md` §4a exige que un defecto de una **capability de plataforma** —`Storage`, `Identity`,
`Tenancy`, `Authorization`, `Audit`, `Notifications`— se registre acá con evidencia de código
antes de corregirse. Corregir en silencio está prohibido; corregir **no** lo está.

| ID | Qué | Evidencia | Estado |
| --- | --- | --- | --- |
| `SDD-OD-22` | **`SearchAsync` no podía filtrar por dueño.** `OwnerId` y `OwnerType` se escribían al crear el archivo y se devolvían en `FileResourceResponse`, pero no había forma de consultar por ellos: el dato entraba y no salía | `IFileResourceRepository.SearchAsync` recibía `tenantId`, `search`, `status`, `kind`, `category`, `tag` y paginación — sin `OwnerId`. Y ninguno de los cuatro índices de `file_resources` servía para filtrarlo | **Cerrada 2026-08-18 por `CAT-09`**, que agregó el filtro, el índice `IX_file_resources_tenant_owner` y su migración. Se registra **retroactivamente**: el slice la corrigió sin haberla registrado antes, que es la mitad de §4a que faltó |
| `SDD-OD-23` | **El filtro `status` del listado de archivos cae en silencio.** `?status=Basura` devuelve la lista **sin filtrar**, como si no se hubiera pedido nada | `StorageEndpoints.cs`, `ListAsync`: `Enum.TryParse` sin `Enum.IsDefined`, con `?? null` como salida. Es el mismo fallback que `CAT-05` corrigió en el `POST`, donde un `ownerType` inválido se volvía `User` y devolvía `201` | **Abierta.** No bloquea a nadie hoy. Cambiar el contrato de un filtro preexistente necesita decisión del owner; el `POST` sienta el precedente de que un valor inválido debe fallar. Señalizado en el código |
...
**El owner pidió abrir `storage` como módulo, siguiendo lo que este mismo ledger recomendaba. Se
leyó el método antes de ejecutarlo, y el método dice que no.**

`00-metodo/apertura-de-modulo.md` tiene una sección «Cuándo NO abrir un módulo» con cuatro señales,
y la **primera** describe exactamente este caso:

> | Es infraestructura que varios módulos usan igual | Capability de plataforma. Se consume, no se crea |
>
> Las cuatro salieron de casos reales de este proyecto.

Y `01-contexto/registro-de-modulos.md` nombra a `Storage` en la lista de capabilities: «No son
módulos de este proyecto. Se **consumen**, no se reescriben».

**El error fue propio, y estaba en dos documentos.** El spec de `CAT-09` y este ledger declaraban
que `Storage` «sigue sin ficha, sin gate y sin prefijo» como si fuera una carencia, y que «la
tercera vez debería abrir el módulo». Se escribió sin verificar `apertura-de-modulo.md`. Los dos
documentos quedaron corregidos, con el bloque citado en cada uno.

**Lo que sí faltaba, y era real.** `AGENTS.md` §4a — «Defecto en una capability» — pide cuatro
cosas cuando un slice choca con un hueco de una capability. `CAT-09` cumplió tres: corrección
acotada dentro del slice, `Repos afectados` declarado, y evidencia RED **en el proyecto de pruebas
de la capability** (`Modules.Storage.UnitTests` y `Modules.Storage.IntegrationTests`, no sólo en
quien la descubrió). **La que faltó es la primera: registrar el defecto como `SDD-OD-*` con
evidencia de código.** Se corrige con `SDD-OD-22` —el hueco de `SearchAsync`, cerrada
retroactivamente por `CAT-09`— y `SDD-OD-23` —el fallback silencioso del filtro `status`, abierta—,
en la sección «Decisiones abiertas» que este ledger no tenía y ahora sí.

**Un detalle de numeración que casi cuesta una colisión.** `SDD-OD-*` es una **serie global entre
los dos repos**. Los primeros IDs libres parecían ser el 17 y el 18 mirando `develop` del
frontend — pero `git log --all -S` mostró que el 17 al 21 ya estaban tomados por el módulo
`account`, en una rama sin mergear. Los IDs no se reasignan, así que se tomaron **22 y 23**. Para
la próxima: buscar el máximo en **todas las ramas de los dos repos**, no en la rama publicada.

**Lo que esto no cambia:** ni una línea de código. `CAT-09` sigue `Complete` y correcto; lo que
estaba mal era cómo dos documentos describían su relación con el método.

### 2026-08-18 — `CAT-09`: el dato que entraba y no salía

**Sale de un error de spec, no de un pedido.** `CAT-05` dejó la galería fuera de alcance
afirmando que «`Storage` ya responde qué archivos son de este producto por `OwnerId`/`OwnerType`».
Se verificó el 2026-08-16 y era falso: los dos campos se escribían al crear el archivo y se
devolvían en la respuesta, pero `SearchAsync` **no filtraba por ellos**. La exclusión de alcance
era correcta —`CAT-05` era la portada— pero la justificación no.

**Lo que no se veía desde afuera, y cambió el tamaño del slice:**

| Hallazgo | Consecuencia |
| --- | --- |
| Ninguno de los cuatro índices de `file_resources` sirve para filtrar por dueño —son por status, categoría y tags— | Índice nuevo y **migración**. Dejó de ser «agregar un `Where`» |
| El filtro `status` del mismo listado cae en silencio ante un valor inválido: `?status=Basura` devuelve la lista **sin filtrar** | Es el mismo fallback que `CAT-05` corrigió en el `POST`. **No se corrige** —contrato preexistente, ningún criterio lo pide— pero el filtro nuevo **no lo repite**, y queda señalizado en el código |

**Una decisión que la tuvo que tomar el RED.** Dos pruebas fallaron en el primer GREEN y no era un
defecto: era una ambigüedad que nadie había resuelto. `?ownerId=X&ownerType=` —el tipo presente
pero vacío— ¿es inválido o ausente? Ganó **ausente**: decirle a alguien «el tipo no es válido» lo
manda a revisar un valor que nunca escribió. Se corrigió **la prueba**, no el código, y la razón
quedó escrita en el test.

**La verificación contra el mecanismo ausente, que acá era obligatoria.** `CA-CAT-09-06` es
frontera de aislamiento entre tenants. Sin `resource.TenantId == tenantId` en el `Where`:
...
**Ese `1` es el archivo del tenant B en la lista del tenant A.** El riesgo es real y no teórico: un
`OwnerId` es único de por sí, así que una implementación que filtrara **sólo** por él pasaría todas
las demás pruebas del archivo y publicaría la biblioteca ajena en cuanto dos tenants compartieran
un id. El runtime lo montó a propósito para que pudiera fallar: se creó en el tenant B un archivo
con **el mismo `ownerId`** que el producto del tenant A.

**El gate se escribió distinto, y a propósito.** La ruta es `/api/v1/tenants/{tenantId}/files`, que
**la expone `Storage`, no `catalog`**. Meterla entre las trece operaciones de la tabla habría
mentido sobre qué módulo la sirve, así que va en sección aparte con la deuda declarada.

**El fallo de verificador que se repitió dos veces, no se repitió una tercera.** El `%{http_code}`
de `curl.exe` se captura en su **propia invocación**, no dentro de un array de argumentos de
PowerShell — que es lo que lo truncaba a `4` en `CAT-07` y `CAT-08`. Los 9 criterios de runtime
salieron a la primera.

**Y un detalle de entorno que ahorra una vuelta:** `dotnet ef database update` falla contra la base
local porque el design-time factory apunta a `5432` y ésta vive en `5433`. **No hace falta la
cadena de conexión** —que es secreto—: cada módulo tiene su `<Modulo>DatabaseInitializer` con
`MigrateAsync`, así que levantar la API aplica las migraciones pendientes.

### 2026-08-17 (cont.) — `CAT-08`: la tasa que no se podía editar, ni borrar, ni reactivar

**Sale de la deuda que `CAT-07` declaró al cerrar**, no de un pedido nuevo. Y al escribir el spec
resultó que no era «la misma asimetría que `Product`»: era **peor**, y la había causado `CAT-06`
sin querer.

**El atolladero, en una tabla.** Una tasa inactiva **que algún producto usa** respondía:

| Operación | Respuesta | Por qué |
| --- | --- | --- |
| `PUT` | 422 `catalog.tax_rate.inactive` | `Update` abre con `EnsureActive()` |
| `DELETE` | 422 `catalog.tax_rate.in_use` | La FK es `RESTRICT` y hay productos apuntándole |
| `deactivate` | 422 `already_inactive` | Ya está inactiva |
| reactivar | no existía | Es este slice |

**Ninguna salida por la API.** `CAT-06` cerró la puerta de adelante dando por hecho que la de atrás
estaba abierta. Y desactivar por error es fácil justamente porque **no duele en el momento**:
`ProductTaxRateResolver` acepta tasas inactivas a propósito, así que los productos siguen
funcionando y nada avisa. El problema aparece semanas después, cuando alguien quiere corregir el
porcentaje.

**`CA-CAT-08-10` reproduce el atolladero entero antes de resolverlo** —el `PUT` en 422, el `DELETE`
en 422, y recién ahí la activación y el `PUT` que ahora pasa—. El lente la revisó específicamente
buscando teatro y concluyó que no lo es: quitar el guard de `Update` **o** el `RESTRICT` del
`DELETE` la pone roja antes de llegar al paso de activación.

**La verificación contra el mecanismo ausente, con el detalle que la hace valer.** Sin `Version++`,
`ActivateAdvancesTheConcurrencyToken` da `Expected: 3 / Actual: 2` — y **las otras tres pruebas de
`Activate` quedan verdes**. Eso es exactamente lo que justifica que esa prueba exista por separado.

**La revisión encontró algo, y no era un bug.** Cero bloqueantes, pero una `SUGGESTION` que se
aplicó: `CA-CAT-08-07` probaba que el **422** no deja rastro en el outbox, pero el camino del
**403** sólo se verificaba en runtime. Si alguien moviera el `auditPublisher.Publish` por encima de
la autorización, aparecería una fila de auditoría de una operación que nunca ocurrió y la prueba
seguiría en verde. **Era un agujero de detección, no de comportamiento** — el código estaba bien,
la red que lo sostiene no. Se agregó la aserción y el lente reevaluó: cierra el hueco sin
introducir nada.

**Un error propio, detectado antes de publicar.** El primer candidato de revisión traía **9**
archivos en vez de 7: se habían colado `.atl/skill-registry.md` y su cache, que el owner había
decidido explícitamente dejar fuera de los commits. Como no estaba publicado, se rehicieron los dos
commits sin ellos y se rehizo la revisión sobre el candidato limpio. **La lección no es «mirar el
`git add`»**: es que el manifiesto de rutas del candidato es la última oportunidad de ver qué se
está por publicar, y hay que leerlo.

**Y la regla que `CAT-07` dejó escrita, cumplida.** Revisión y `validate --gate pre-push` **antes**
del merge y el push:
...
Contra el `allow` vacío de `CAT-07` —«the publication range is empty»— que no gobernó nada.

**El mismo fallo de verificador, repetido.** `CA-CAT-08-06` reportó `4` en vez de `403` en runtime:
el `%{http_code}` de `curl.exe` se trunca al capturarlo dentro de un array de argumentos de
PowerShell. **Es idéntico al de `CAT-07`**, y volvió a pasar porque se reusó el script. El código
nunca estuvo mal; el medidor sí, dos veces. Verificado con `curl.exe` directo: `activate=403`.

**Estado del módulo:** `catalog` quedó **simétrico**. `Product` y `TaxRate` tienen los dos su
`activate`, y ninguna entidad del módulo tiene ya un estado terminal sin salida.
...
**No es un slice y no toca código.** Salió de una pregunta del owner: *«por qué al crear un producto
se recibe `imageFileId`, si la idea es que un producto tenga varias fotos»*.

**La respuesta corta es que `imageFileId` nunca fue «la imagen»: es la portada**, y eso está decidido
en `CAT-05` —la galería es de `Storage`, que guarda a qué producto pertenece cada archivo por
`OwnerId`/`OwnerType`. Hasta ahí el spec estaba bien.

**Lo que no estaba bien es cómo lo justificó.** El ítem de «Fuera de alcance» afirmaba que «`Storage`
ya responde qué archivos son de este producto por `OwnerId`/`OwnerType`». Se verificó contra el
código y **es falso**:

```txt
IFileResourceRepository.SearchAsync(tenantId, search, status, kind, category, tag, page, pageSize)
```

`OwnerId` **no está entre los filtros**. Los dos campos se escriben al crear el archivo
(`CreateFileRequest`) y se devuelven en `FileResourceResponse`, pero no hay forma de **consultar**
por ellos: el dato entra y no sale. Un producto puede tener 10 fotos marcadas como suyas y ningún
endpoint las lista.

**Qué se corrigió, y qué no.** La exclusión de alcance de `CAT-05` **sigue en pie** —ese slice era
la portada, no la galería— así que no se reabre nada. Lo que se corrigió es la razón, con el mismo
patrón de bloque citado que el propio `CAT-05` ya usó para el enum de `FileOwnerType` el
2026-08-15: se transcribe lo que decía, se dice por qué es falso y qué conclusión **no** cambia.
Se agregó además la fila correspondiente en «Lo que este slice deja abierto, y no es suyo».

**`SDD-ADR-01` funcionando como corresponde:** gana el código, y el documento se corrige. Vale
anotar de dónde vino el error —una capacidad de **otro módulo** que se dio por existente sin
abrirla—, que es justo el tipo de afirmación que el método pide verificar y esta vez no se
verificó.

**Lo que queda abierto, y no se abrió:** la galería necesita su propio slice —filtro por owner en
`SearchAsync` y su query param en el listado de `Storage`—, **sin ID todavía**, porque `CAT-07`
está `In Progress` y en este repositorio corre **un slice a la vez**.

### 2026-08-16 (cont.) — «Estado global» decía «Ninguno» con `CAT-07` abierto

**La contradicción:** el campo «Slice activo» decía **Ninguno**; la tabla de slices tenía `CAT-07`
en **`In Progress`** desde el 2026-08-15. Se verificó cuál de los dos era cierto en vez de elegir.

**Gana la tabla: `CAT-07` está abierto, y sin una línea de código.** Tres evidencias, todas del
2026-08-16:

```txt
$ rg -n "Activate" src/Modules/Catalog -i        -> sólo Deactivate. Ningún ActivateProduct.cs,
                                                    ningún endpoint /activate, ninguna prueba
$ tail sdd/03-modulos/catalog/slices/CAT-07-*.md -> "Evidencia de cierre: _Pendiente — el slice
                                                    está abierto._"
$ git status --porcelain                         -> ?? sdd/03-modulos/catalog/slices/CAT-07-...
```

**De dónde salió el error, que es lo que conviene no repetir.** El campo se escribió al cerrar
`CAT-06` —«Ninguno… y no se abrió nada nuevo», que era cierto en ese momento— y después, **en la
misma sesión**, se abrió `CAT-07` actualizando la tabla de slices y el spec, pero no «Estado
global». El spec de `CAT-07` además quedó **sin commitear**, así que en un clon nuevo el único
rastro del slice abierto era una fila de tabla contra un campo que decía lo contrario.

**Lo que se corrigió, además del campo.** Al mirarlo de cerca, «Estado global» tenía **tres** filas
sin rotar desde los cierres del 2026-08-15, no una:

| Campo | Decía | Dice |
| --- | --- | --- |
| Slice activo | `Ninguno` | **`CAT-07`**, `In Progress`, sin código |
| Último slice cerrado | `CAT-04` | **`CAT-06`** — `CAT-05` y `CAT-06` cerraron después, el mismo día |
| Slice anterior | `CAT-03`, con estado `In Progress` | **`CAT-05`** — y `CAT-03` es `Complete` desde el 2026-08-13 |

El párrafo de arriba del cuerpo, que abría con «No hay slice activo en este repositorio», también
se corrigió. **`Último slice completado` (`CAT-02`) se dejó intacto a propósito:** dice «el
primero cerrado en este ledger», o sea que es un dato histórico y no un campo rotativo, aunque su
nombre se pise con el de arriba. Renombrarlo o rotarlo sería rediseñar el ledger, y eso no es
esta corrección.

**Ninguna evidencia se perdió al rotar:** las celdas viejas de `CAT-04` y `CAT-03` repetían lo que
ya está en sus filas de la tabla de slices y en sus entradas de handoff.

**Lo que esto deja como pendiente y no es de esta corrección:** el spec de `CAT-07` sigue sin
commitear. Va con el commit de quien trabaje el slice.
