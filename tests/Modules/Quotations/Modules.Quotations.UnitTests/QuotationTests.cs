...
public sealed class QuotationTests
{
    ...
    // convertir, sin recrear la venta entera. Sólo sumar — ver Sale.RecalculatePaymentStatus
    // para la otra mitad de esta historia.
    [Fact]
    public void AddItemAfterConversionAddsALineToAConvertedQuotation()
    {
        var quotation = ConvertibleSentQuotation();
        quotation.ConvertToSale(AdvisorId, Now);
        var totalBefore = quotation.Total;
        var productId = Guid.CreateVersion7();
    ...
    // (el campo se agregó ese día, sin backfill), así que hay ventas Pending reales cuya
    // cotización quedó en Sent para siempre. Exigir Converted acá las dejaba sin poder
    // editarse — el caso de uso ya encontró la venta pendiente antes de llamar, y eso alcanza.
    [Fact]
